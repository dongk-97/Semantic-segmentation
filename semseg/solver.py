...
class Solver(object):
    ...
    def build_model(self):
        ...
        # Build criterion
        self.criteria["bce_dice"] = loss  # Binary cross entropy
    ...
    def backward(self, phase="train"):
        ...
        # Loss defition
        bce_dice_loss = self.criteria["bce_dice"].DiceBCELoss(self.weights)(self.outputs, self.labels)
        ...
        loss = bce_dice_loss + l1_loss
        if phase == "train":
            loss.backward()

        self.loss["bce_dice"][phase].append(bce_dice_loss.item())
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            for i, (images, labels, weights) in enumerate(self.image_loader["test"]):
                ...
                axes[0].imshow(images[0][0].detach().cpu().numpy(), cmap="gray")
                axes[1].imshow(labels[0][1].detach().cpu().numpy())
                axes[2].imshow(outputs[0][1].detach().cpu().numpy())

                fig.set_dpi(300)
                fig.tight_layout()
                fig.subplots_adjust(hspace=0.01, wspace=0.01)
                fname = 'result' + str(i) + '.jpg'
